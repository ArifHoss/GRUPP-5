...
const completedTasks = computed(() => {
  ...
  return userStore.currentUser.completedTasks.filter(
    (task) => task.dateCompleted === selectedDate.value,
  )
})
...
const daysInMonth = computed(() => {
  ...
  // Fill empty slots before first day to align correctly
  for (let i = 0; i < firstDay.getDay(); i++) {
    days.push(null) // Ensure null values for empty days
  }

  // Add actual dates correctly
